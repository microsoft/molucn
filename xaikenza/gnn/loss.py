...
import numpy as np
from xaikenza.utils.utils import get_common_nodes, get_positions, get_substituent_info, get_substituents
...
def loss_uncommon_node(
    ...
    loss = F.mse_loss(
        delta_emb, delta_y, reduction=reduction
    )
    return loss
...
def loss_uncommon_node_local(
    ...
    Compute the loss that correlates decoration embeddings and activity cliff.
    """
    idx_common_i, idx_common_j, map_i, map_j = get_common_nodes(data_i.mask.cpu(), data_j.mask.cpu())
