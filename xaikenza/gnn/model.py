...
from xaikenza.gnn.aggregation import AttentionalAggregation
from xaikenza.utils.train_utils import overload
import torch
...
class GNN(torch.nn.Module):
    ...
    def get_gap_activations(self, x, edge_index, edge_attr, batch):
        ...
        Assumes there is a global average pooling (GAP) layer to produce the
    ...
    def get_substituent_rep(self, sub, data):
        node_x = self.get_node_reps(data.x, data.edge_index, data.edge_attr, data.batch)
        bool_mask = torch.zeros(data.mask.size(), dtype=torch.bool, device=data.x.device)
        bool_mask[sub] = 1
        masked_batch = data.batch[bool_mask]
        bs = len(torch.unique(data.batch))
        masked_bs = len(torch.unique(masked_batch))
        assert masked_bs==1
        unique_batch = torch.zeros(data.mask.size(), dtype=torch.long, device=data.x.device)
    ...
    def get_uncommon_graph_rep(self, data):
        node_x = self.get_node_reps(data.x, data.edge_index, data.edge_attr, data.batch)
