...
w_model_res = pd.merge(
    w_model_res, df_stats_350[["target", "weight", "n_pairs"]], on="target", how="left"
)
w_model_res.columns
# %%
...
# %%
w_model_res[["loss", "rmse_test", "pcc_test"]].groupby("loss").mean()[
    ["rmse_test", "pcc_test"]
]
# %%
w_model_res[["loss", "rmse_test", "pcc_test"]].groupby("loss").std()[
    ["rmse_test", "pcc_test"]
]
# %%
w_stats = (
    w_model_res[["loss", "w_rmse_test", "w_pcc_test"]]
    .groupby("loss")
    .sum()[["w_rmse_test", "w_pcc_test"]]
)
...
print(
    "stats rmse weighted: RF vs GNN: ", stats.ttest_rel(rmse_scores_rf, rmse_scores_gnn)
)
print(
    "Number of targets for which GNN rmse < RF rmse: ",
    np.sum([np.array(rmse_scores_rf) - np.array(rmse_scores_gnn) > 0]),
...
print(
    "stats rmse: GNN MSE+UCN vs GNN MSE: ",
    stats.ttest_rel(rmse_scores_mse, rmse_scores_mse_ucn),
)
print(
