...
from molucn.filter import get_num_cmn_sites
...
def get_global_directions_filtered(
    ...
    accs_one_site, accs_multi_sites = [], []
    for k in range(len(pairs_list)):
        ...
        if n_cmn_sites == 1:
            accs_one_site.append(mcs_accs)
        else:
            accs_multi_sites.append(mcs_accs)
    return np.array(accs_one_site), np.array(accs_multi_sites)
